...
links: [forgetting-curve]      # GENERATED by /okf:reindex from body [[id]] — do not hand-edit
...
3. Write the body. Link related concepts inline with `[[other-id]]` wiki-links.
   The `links:` frontmatter field is **generated** from these body wiki-links by
   `/okf:reindex` — do not hand-edit it. Body `[[id]]` references are the source
   of truth for the link graph.
4. Cite sources in the `sources` list (each with at least a `title`).
5. Suggest the user run `/okf:reindex` to refresh `index.md` and regenerate
   `links:`, then `/okf:log` to record the change.
...
`updated >= created`; every body `[[id]]` resolves to an existing concept (ERROR);
and `links:` matches the body wiki-links (WARN if out of sync — run `/okf:reindex`).
Dangling body `[[id]]` references and an out-of-date `index.md` are the most common
failures.
...
- `/okf:reindex [bundle-path]` — regenerate `links:` from body `[[id]]` and rebuild `index.md` (`scripts/reindex.py`).
- `/okf:log [message]` — append a `log.md` entry (`scripts/append-log.py`).
- `/okf:rename <old-id> <new-id>` — link-aware, recoverable concept rename (updates all referencing concepts and reindexes).
- `/okf:query` — filter concepts by `--tag`, `--type`, `--status`, or `--text` (all filters AND-combined).
