...
links: [forgetting-curve]      # GENERATED by /cc-okf:reindex from body [[id]] — do not hand-edit
...
`/cc-okf:rename <old-id> <new-id>` — it updates the file, the `id`, all referencing
...
3. Write the body. Link related concepts inline with `[[other-id]]` wiki-links.
   ...
   `/cc-okf:reindex` — do not hand-edit it. Body `[[id]]` references are the source
   of truth for the link graph.
4. Cite sources in the `sources` list (each with at least a `title`).
5. Suggest the user run `/cc-okf:reindex` to refresh `index.md` and regenerate
   `links:`, then `/cc-okf:log` to record the change.
...
and `links:` matches the body wiki-links (WARN if out of sync — run `/cc-okf:reindex`).
...
`/cc-okf:validate` enforces it programmatically.
...
- `/cc-okf:reindex [bundle-path]` — regenerate `links:` from body `[[id]]` and rebuild `index.md` (`scripts/reindex.py`).
