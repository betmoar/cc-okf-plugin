...
- [ ] **Step 6: Ignore Python bytecode, then commit**

Append to `.gitignore` (so test runs don't litter `scripts/__pycache__`):
...
```bash
git add .gitignore scripts/okf_common.py tests/okf_test_util.py tests/test_okf_common.py
...
- Produces:
  - `run(bundle: str, dry_run: bool = False) -> int`
  - `regenerate_links(bundle: str, dry_run: bool) -> int` (knowledge write; called by `rename.py` before the file rename)
  - `rebuild_index(bundle: str, dry_run: bool = False) -> int` (regenerable; called by `rename.py` after the file rename)
...
**Interfaces:**
- Consumes: `okf_common.{ID_RE, set_id, parse_frontmatter}`, `reindex.{regenerate_links, rebuild_index}`
- Produces:
...
```
Expected: PASS (all eight, both invocations)
...
canonical; `links:` is generated." Add a short migration note: on first v0.2
reindex, hand-authored `links:` entries with no backing body `[[id]]` are dropped
(see the migration notice); and any dict-form `links:` entries (an undocumented
v0.1 shape) are not carried over — express links as body `[[id]]` wiki-links.
In `references/conformance.md`: dangling
...
Expected: passes (no schema errors in the two new command files). If this CLI
build rejects `--strict`, fall back to `claude plugin validate .` — the gate is a
clean manifest parse, not the specific flag.
...
- §A splicer contract — **every form pinned** in Task 1: absent (`test_insert_when_absent`), inline (`test_replace_inline`), block (`test_replace_block`), empty `[old]` and `[]` removal (`test_empty_removes_field`, `test_empty_removes_inline_empty_form`), comment-in-block swallow (`test_comment_in_links_block_is_swallowed`), trailing/blank-line preservation (`test_blank_line_after_links_block_preserved`), mid-frontmatter placement (`test_placement_preserved_when_field_mid_frontmatter`), idempotent + preserve-bytes. ✓
- §A migration notice → Task 2 `test_migration_notice_for_dropped_links`. ✓
- §A reindex rebuilds index.md → Task 2 `test_rebuilds_index`. ✓
...
- §B rename (rename-last, idempotent, token-exact, recovery, updated-untouched, id/filename-match, referrer-links+index regen) → Task 4, all pinned by tests. ✓
- §C query (AND filters) → Task 5. ✓
- §D shared module → Task 1. ✓
- §E docs/positioning (+ dict-form migration note) → Task 6. ✓
...
- v0.1 regression guard (required-fields, index-staleness still work post-refactor) → Task 3 `test_required_field_check_survives_refactor`, `test_index_staleness_still_warns`. ✓
...
**Design decision (validate ↔ reindex consistency):** the stale-`links:` check (Task 3) uses **list** (order-sensitive) comparison, not set. Reindex generates `links:` in body first-appearance order, so order is canonical; list comparison makes validate WARN *exactly when* reindex would rewrite the file. Set comparison was rejected — it would call a file clean that reindex then mutates. // DECISION: keep list comparison.

**Spec refinement (rename × reindex ordering):** spec §B lists "step 4: reindex, step 5: rename." The plan splits reindex into `regenerate_links` (run before the rename — it writes `links:` into concept files, a knowledge mutation that must be recoverable) and `rebuild_index` (run after the rename — otherwise index rows would point at `<old>.md`). This preserves the spec's intent (all knowledge writes before the atomic rename; index is regenerable) while fixing a stale-index-path defect the original single-call ordering would have shipped. // DECISION: regen-links → os.rename → rebuild-index.
